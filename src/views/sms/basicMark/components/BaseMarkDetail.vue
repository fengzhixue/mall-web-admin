<template> 
 ...
 <el-form-item label="优惠设置：" v-show="baseMark.smallType===1">
       ...
       <el-form-item label="优惠设置：" v-show="baseMark.smallType===2">
...
<script>
  ...
  import {createSmsBasicMarking, deleteSmsBasicMarking, getSmsBasicMarking,updateSmsBasicMarking} from '@/api/sms/baseMark'
...
memberLevelList:[],
 ...
 props: {
    ...
    created(){
      ...
      this.isEdit = true;
        getSmsBasicMarking(this.$route.query.id).then(response => {
        ...
        this.$refs[formName].validate((valid) => {
          if (valid) {
            ...
            this.$confirm('是否提交数据', '提示', {
              ...
              if (this.isEdit) {
                updateSmsBasicMarking(this.$route.query.id, this.baseMark).then(response => {
                ...
                createSmsBasicMarking(this.baseMark).then(response => {
                  this.$refs[formName].resetFields();
                  this.baseMark = Object.assign({},defaultHomeAdvertise);
                  this.$message({
                    message: '提交成功',
